...
# --- GLOBAL STATE ---
$script:FoundLinks = @()      # All links found by the scanner
$script:SelectedLinks = @()   # Links staged for removal
...
function Get-GlobalConfiguration {
    ...
    $TxtTenant.Location = "20, $($Y+20)"; $TxtTenant.Size = "480, 25"; $TxtTenant.Text = "Enter Azure Tenant ID"
    ...
    $TxtApp.Location = "20, $($Y+20)"; $TxtApp.Size = "480, 25"; $TxtApp.Text = "Enter Application (Client) ID"
    ...
    $TxtSecret.Text = "Enter Application Secret"
...
function Show-InteractiveRemovalPrompt {
    ...
    # --- TOP SECTION: Found Links & Filters ---
    ...
    function Update-Grids {
        ...
        foreach ($item in $script:SelectedLinks) {
            ...
            $GridSelected.Rows[$index].Cells["Select"].Value = $true # It's in the staged area, so it's checked
    ...
    # --- EVENT HANDLERS FOR MOVING ITEMS ---
    # When user clicks a checkbox in GridFound
    ...
    # When user unclicks a checkbox in GridSelected (Moving back to found)
    ...
    # To ensure immediate reaction to checkbox clicks
    ...
    $BtnRevoke.Add_Click({
        ...
        if ([System.Windows.Forms.MessageBox]::Show($msg, "Confirm Bulk Revocation", "YesNo") -eq "Yes") {
            ...
            $script:SelectedLinks = @() # Clear staged area for processing
            ...
            foreach ($item in $toProcess) {
                try {
                    ...
                    $script:SelectedLinks += $item # Put it back if it failed
                }
            }
