...
# --- 1. PROCESS SHAREPOINT SITES (DOCUMENT LIBRARIES) ---

Write-Host "`n=== Starting SharePoint Site Cleanup (Tenant-Wide) ===`n" -ForegroundColor Cyan

# Fetch all sites in the tenant using -All
try {
    Write-Host "Fetching all sites across the tenant (This may take time for large tenants)..." -ForegroundColor DarkYellow
    # If the 'Found 0 sites' issue persists, the switch to Application Permissions will fix it.
    $sites = Get-MgSite -All | Select-Object Id, DisplayName, WebUrl -ErrorAction Stop
    Write-Host "Found $($sites.Count) sites to check." -ForegroundColor Green
} catch {
    Write-Error "Failed to fetch SharePoint Sites. Ensure you have 'Sites.FullControl.All' scope consented. Error: $($_.Exception.Message)"
    exit 1
}

foreach ($site in $sites) {
    Write-Host "`nProcessing SharePoint Site: $($site.DisplayName) - $($site.WebUrl)" -ForegroundColor Yellow
    
    try {
        # Get all document libraries (drives) for the site
        $drives = Get-MgSiteDrive -SiteId $site.Id -All -ErrorAction Stop
    } catch {
        Write-Warning "Failed to get drives for site $($site.DisplayName). Skipping."
        continue
    }

    foreach ($drive in $drives) {
        Write-Host "  -> Processing Drive (Library): $($drive.Name) (ID: $($drive.Id))" -ForegroundColor Green
        # Start recursion from the root of the drive
...
# --- 2. PROCESS ALL USERS' ONEDRIVES (TENANT-WIDE FIX) ---

Write-Host "`n=== Starting Tenant-Wide OneDrive Cleanup ===`n" -ForegroundColor Cyan

# 1. Fetch ALL users in the tenant
try {
    # We only need the User Principal Name (UPN) to target their drive
    Write-Host "Fetching all users in the tenant..." -ForegroundColor DarkYellow
    $allUsers = Get-MgUser -All | Select-Object UserPrincipalName -ErrorAction Stop
    Write-Host "Found $($allUsers.Count) users to check." -ForegroundColor Green
} catch {
    Write-Error "Failed to fetch all users. Ensure 'Directory.Read.All' and 'User.Read.All' scopes are consented. Error: $($_.Exception.Message)"
    $allUsers = @()
}
